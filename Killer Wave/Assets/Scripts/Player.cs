...
public class Player : MonoBehaviour, IActorTemplate
{
    ...
    void Movement()
    {
        if (Input.GetAxisRaw("Horizontal") > 0)
        {
            ...
            {
                transform.localPosition += new Vector3(Input.GetAxisRaw("Horizonal") * Time.deltaTime * travelSpeed, 0, 0);
            }
        }
        if (Input.GetAxisRaw("Horizontal") < 0)
        {
        ...
        if (Input.GetAxisRaw("Vertical") < 0)
        {
            ...
            {
                transform.localPosition += new Vector3(0, Input.GetAxisRaw("Veritcal") * Time.deltaTime * travelSpeed, 0);
            }
        }
        if (Input.GetAxisRaw("vertical") > 0)
        {
